...
with aba_cadastro:
    ...
    # Identificar chaves unicas de recuperados para calcular fechadas reais
    ...
    with st.form("form_ace", clear_on_submit=True):
        ...
        with col1:
            ...
            quarteirao = st.text_input("Número do Quarteirão", placeholder="Ex: 142")
        ...
        if submitted:
            if not quarteirao or not rua or not numero_imovel:
                st.warning("⚠️ Por favor, preencha Quarteirão, Rua e Número do Imóvel.")
            else:
                nova_vistoria = {
...
with aba_busca:
    st.subheader("🔍 Gerenciamento Avançado & Recuperação Rápida")
    st.markdown("Filtre seus dados abaixo ou use o painel interativo de **Imóveis Fechados** para marcá-los diretamente como recuperados ao retornar do campo.")
    ...
    if not st.session_state.vistorias:
        ...
        with col_f1:
            ...
            filtro_quarteirao = st.selectbox("Quarteirão", ["Todos"] + quart_unicos, key="f_quart")
        with col_f2:
        ...
        with col_f3:
            ...
            filtro_data = st.selectbox("Data da Visita", ["Todas"] + datas_unicas, key="f_data")
        with col_f4:
            filtro_status = st.selectbox("Status", ["Todos", "Apenas Fechadas Pendentes", "Recuperadas", "Normal"], key="f_status")
        ...
        if filtro_quarteirao != "Todos":
            df_filtrado = df_filtrado[df_filtrado["Quarteirao"] == filtro_quarteirao]
        if filtro_rua != "Todas":
        ...
        if filtro_status == "Apenas Fechadas Pendentes":
            df_filtrado = df_filtrado[(df_filtrado["Visita"] == "Fechada") & (~df_filtrado["Chave_Imovel"].isin(imoveis_recuperados_set))]
        ...
        st.write("---")
        st.markdown(f"### 📋 Lista Filtrada ({len(df_filtrado)} registros)")
        if not df_filtrado.empty:
            st.dataframe(df_filtrado[["Data", "Quarteirao", "Lado", "Rua", "Numero", "Tipo Imovel", "Visita"]], use_container_width=True)
        else:
            st.success("🎉 Nenhum imóvel encontrado com estes filtros!")

        # PAINEL INTERATIVO DE RECUPERAÇÃO RÁPIDA POR CHECKBOX
        st.write("---")
        st.subheader("⚡ Painel Interativo: Marcar Fechadas como Recuperadas")
        st.markdown("Marque a caixa de seleção ao lado da casa fechada que você acabou de recuperar. Ela sairá instantaneamente da lista de pendências!")

        # Filtrar apenas as casas que estão atualmente com status Fechada e ainda não recuperadas
        ...
        if not df_pendentes_painel.empty:
            for idx, row in df_pendentes_painel.iterrows():
                ...
                with col_info:
                    st.markdown(f"**Quarteirão:** {row['Quarteirao']} | **Rua:** {row['Rua']}, **N° {row['Numero']}** (Tipo: {row['Tipo Imovel']} - Registrado em: {row['Data'])")
                with col_check:
                    # Checkbox interativo para virar recuperada
                    marcado = st.checkbox("Recuperar", key=f"rec_{idx}")
                    if marcado:
                        # Atualiza o status diretamente no session_state
                        st.session_state.vistorias[idx]["Visita"] = "Recuperada"
                        st.success("Atualizado!")
                        st.rerun()
        else:
            st.success("✨ Parabéns! Não há imóveis fechados pendentes para recuperar no momento.")

with aba_backup:
    st.subheader("🔒 Central de Segurança e Backup Manual")
    st.markdown("Proteja seus dados! Clique no botão abaixo para baixar um arquivo de segurança com todas as suas vistorias diretamente para o seu celular.")

    if st.session_state.vistorias:
        dados_json = json.dumps(st.session_state.vistorias, ensure_ascii=False, indent=4)
        ...
        st.success("💡 Dica: Após salvar o arquivo de backup, guarde-o no seu Google Drive ou envie para você mesmo no WhatsApp!")
    else:
        st.info("⚠️ Não há dados cadastrados ainda para gerar o backup de segurança.")
