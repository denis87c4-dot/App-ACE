  ...
  if st.session_state.vistorias:
    ...
    if not df_fechados.empty:
      ...
      with c_filtro0:
        ...
        data_filtro = st.selectbox("Filtrar por Data da Visita", datas_f, key="filtro_data_fechados")
      with c_filtro1:
      ...
      if data_filtro != "Todas as Datas":
        df_filtrado_fechados = df_filtrado_fechados[df_filtrado_fechados["Data"] == data_filtro]
      ...
      f1.metric("🚪 Total Fechadas / Recusas", len(df_filtrado_fechados))
      f2.metric("🏘️ Quarteirões Afetados", df_filtrado_fechados["Quarteirao"].nunique())
      f3.metric("👤 Agentes Envolvidos", df_filtrado_fechados["Agente"].nunique())

      st.markdown("---")
      st.dataframe(df_filtrado_fechados, use_container_width=True)
    else:
      st.success("🎉 Não há registros de imóveis fechados ou recusas no momento.")
  else:
    st.info("ℹ️ Preencha o 'Relatório Diário' para popular este painel.")
...
with aba_semanal:
  ...
  if st.session_state.vistorias:
    df_v = pd.DataFrame(st.session_state.vistorias)
    df_v["Semana"] = pd.to_numeric(df_v["Semana"], errors="coerce").fillna(1).astype(int)
    ...
    ws1.metric("🏠 Visitas", len(df_semana_filtrada))
    ws2.metric("🗑️ Eliminados", int(df_semana_filtrada["Eliminados"].sum()))
    ws3.metric("🧪 Tubitos", int(df_semana_filtrada["Tubitos"].sum()))
    ws4.metric("🛠️ Tratados", int(df_semana_filtrada["Tratados"].sum()))
    ws5.metric("⚖️ Gramas", f"{df_semana_filtrada['Gramas'].sum():.1f}g")
    ...
    st.info("ℹ️ Preencha o 'Relatório Diário' para gerar os dados semanais.")
...
with aba_reconhecimento:
  ...
  if st.session_state.reconhecimento:
    ...
    df_consolidado = (
        ...
        .agg({"Residencias": "sum", "Outros": "sum", "TB": "sum", "Comercio": "sum", "Total": "sum"})
        .reset_index()
    )
    st.dataframe(df_consolidado, use_container_width=True)
  else:
    st.info("ℹ️ Realize registros no 'Relatório Diário' para gerar o reconhecimento.")
...
with aba_backup:
  ...
  with col_b1:
    ...
    if arquivos_existentes and (st.session_state.vistorias or st.session_state.reconhecimento):
  ...
  with col_b2:
    arquivo_upload = st.file_uploader("Selecione o seu arquivo .zip de backup", type="zip")
    if arquivo_upload is not None:
      if st.button("🔄 Confirmar e Recarregar Dados", use_container_width=True, type="primary"):
        try:
          with zipfile.ZipFile(arquivo_upload, "r") as zip_ref:
            zip_ref.extractall(".")
          if os.path.exists(ARQUIVO_VISTORIAS):
            st.session_state.vistorias = pd.read_csv(ARQUIVO_VISTORIAS).to_dict("records")
          if os.path.exists(ARQUIVO_RECONHECIMENTO):
            st.session_state.reconhecimento = pd.read_csv(ARQUIVO_RECONHECIMENTO).to_dict("records")
          salvar_estado_local()
